...
export default function ProductDetailClient({ slug }: { slug: string }) {
  ...
  // Determine the active color object for tinting
  const defaultColorName = product.variants?.colors?.[0]?.name;
  const activeColorObj = product.variants?.colors?.find((c) => c.name === selectedColor);
                  ...
                  {product.images.map((img, idx) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        const matchingColor = product.variants?.colors?.find(c => c.image === img);
                        if (matchingColor) {
            ...
            <div className="max-w-4xl mx-auto text-neutral-600 text-sm sm:text-base leading-relaxed space-y-4">
              {activeTab === "desc" && (
                <>
                  ...
                  <p>{product.additionalInfo || "Dibuat dari material kayu pilihan berstandar tinggi yang tahan lama dan ramah lingkungan."}</p>
                </>
              )}
              {activeTab === "info" && (
                <p>
